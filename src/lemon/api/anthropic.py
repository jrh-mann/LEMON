...
class AnthropicClient:
    ...
    def messages_create(self, *, messages: List[Dict[str, Any]], max_tokens: int, system: Optional[str] = None) -> Any:
        kwargs: Dict[str, Any] = {
        ...
        client = AnthropicFoundry(api_key=self.config.api_key, base_url=self.config.endpoint)
...
def get_anthropic_client() -> AnthropicClient:
    ...
    return AnthropicClient(AnthropicConfig(endpoint=endpoint, deployment_name=deployment_name, api_key=api_key))


def make_request(messages: List[Dict[str, Any]], max_tokens: int = 1024, model: Optional[str] = None, system: Optional[str] = None) -> Any:
...
def make_simple_request(user_message: str, max_tokens: int = 1024, model: Optional[str] = None) -> Any:
    return make_request([{"role": "user", "content": user_message}], max_tokens=max_tokens, model=model)
...
def make_image_request(
    *,
    ...
    media_type_map = {"PNG": "image/png", "JPEG": "image/jpeg", "JPG": "image/jpeg", "WEBP": "image/webp", "GIF": "image/gif"}
