...
import { listWorkflows, getWorkflow, getPublicWorkflow, deleteWorkflow, listPublicWorkflows, voteOnWorkflow } from '../api/workflows'
...
export default function WorkflowBrowser() {
  ...
  const filterBySearch = (workflow: WorkflowSummary) => {
    ...
    return (
      ...
      workflow.tags?.some(t => t.toLowerCase().includes(query))
    )
  }
  ...
  const handleSelectWorkflow = async (workflowSummary: WorkflowSummary, e: React.MouseEvent) => {
    try {
      ...
      // Use the public endpoint for peer-review/published workflows (owned by other users)
      const response = activeTab === 'my-workflows'
        ? await getWorkflow(workflowSummary.id)
        : await getPublicWorkflow(workflowSummary.id)
      const result = hydrateWorkflowDetail(response)
  ...
  if (error) {
    return (
      <div className="workflow-browser error">
        ...
        <button className="ghost" onClick={loadMyWorkflows}>
          Retry
        </button>
      </div>
    )
    ...
    <div key={workflow.id} className="workflow-card-container">
      ...
      >
        ...
        <div className="workflow-tags">
          {workflow.domain && (
            <span className="workflow-tag domain">{workflow.domain}</span>
          )}
