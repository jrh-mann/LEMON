...
export default function Header() {
  ...
  }, [isDirty, openModal, reset, resetChat])
    ...
    <>
      <header className="app-header animate-slide-down-1">
        ...
        <div className="header-actions">
          <button
            className={`ghost${location.pathname === '/library' ? ' active' : ''}`}
            onClick={() => {
              if (location.pathname === '/library') {
                const returnPath = currentWorkflow?.id ? `/workflow/${currentWorkflow.id}` : '/workflow'
                navigate(returnPath)
              } else {
                navigate('/library')
              }
