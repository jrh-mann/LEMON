...
from ..execution.errors import StoppedExecutionError
...
def _purge_stale_executions_locked(now: float) -> None:
    ...
    stale = [
        eid
        for eid, state in _EXECUTION_STATE.items()
...
class SteppedExecutionTask:
    ...
    def _emit_log(
        self, log_type: str, step_info: Dict[str, Any], extra_fields: list
    ) -> None:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.emit_complete(
                success=False, error="Execution stopped by user", path=[]
            )
