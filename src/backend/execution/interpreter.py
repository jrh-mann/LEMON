...
from .errors import StoppedExecutionError
...
class TreeInterpreter:
    ...
    def execute(
        ...
        try:
            ...
            return ExecutionResult(success=False, error=str(e), context=input_values)

        # Get start node
        if "start" not in self.tree:
            return ExecutionResult(success=False, error="Tree missing 'start' node")
            ...
            while current:
                ...
                if on_step is not None:
                    _invoke_on_step(
                        ...
                        warning_message=f"on_step callback error at node '{node_id}'",
                    )
                ...
                if node_type in ("output", "end"):
                    ...
                    if on_step is not None:
                        _invoke_on_step(
                            ...
                            warning_message=f"on_step callback error for end node '{node_id}'",
                        )
                    return ExecutionResult(
                        success=True,
                        output=output_val,
                    ...
                    if node_type == "start" and on_step is not None:
                        _invoke_on_step(
                            ...
                            warning_message=f"on_step callback error for start node '{node_id}'",
                        )
    ...
    def _handle_subprocess_node(
        ...
        sub_input_values = self._map_inputs_to_subworkflow(
            input_mapping, context, subworkflow.inputs, node_label
        )
        ...
        if on_step is not None:
            ...
            _invoke_on_step(
                ...
                warning_message="on_step subflow_start callback error",
            )

            # Create wrapper that adds subflow context to each step
            def _subflow_on_step(step_info: Dict[str, Any]) -> None:
                ...
                _invoke_on_step(
                    ...
                    warning_message="on_step subflow_step callback error",
                )

            # Execute subworkflow with visualization callback
            sub_result = sub_interpreter.execute(
                sub_input_values,
                on_step=_subflow_on_step,
            )
        else:
            # Execute subworkflow without visualization callback
            sub_result = sub_interpreter.execute(sub_input_values, on_step=None)
        ...
        if on_step is not None:
            _invoke_on_step(
                ...
                warning_message="on_step subflow_complete callback error",
            )
    ...
    def _handle_calculation_node(
        ...
        if not operator_name:
            raise InterpreterError(f"Calculation node '{node_label}' missing operator")
        if not operands:
            raise InterpreterError(f"Calculation node '{node_label}' missing operands")
        ...
        for i, operand in enumerate(operands):
            ...
            if kind == "literal":
                ...
                operand_details.append(
                    {"name": str(value), "kind": "literal", "value": float(value)}
                )
                ...
                resolved_operands.append(float(value))
                operand_details.append(
                    {"name": var_name, "kind": "variable", "value": float(value)}
                )
            else:
                raise InterpreterError(
        ...
        try:
            ...
            raise InterpreterError(f"Calculation node '{node_label}' failed: {e}")
        ...
        if on_step is not None:
            ...
            _invoke_on_step(
                ...
                warning_message=f"on_step calculation callback error at node '{node_id}'",
            )
    ...
    def _inject_calculation_output(
        self, output_name: str, output_value: float, context: Dict[str, Any]
    ) -> None:
    ...
    def _inject_subflow_output(
        self, output_variable: str, output_value: Any, context: Dict[str, Any]
    ) -> None:
        ...
        variable_id = self._generate_variable_id(
            output_variable, output_type, "subprocess"
        )
    ...
    def _generate_variable_id(
        self, name: str, var_type: str, source: str = "input"
    ) -> str:
        ...
        for var_id, schema in self.variables_schema.items():
            ...
            if var_type == "number":
                ...
                if not isinstance(value, (int, float)) or isinstance(value, bool):
                    raise InterpreterError(
                        f"{var_id} must be number, got {type(value).__name__}"
                    )

            elif var_type == "bool":
                if not isinstance(value, bool):
                    raise InterpreterError(
                        f"{var_id} must be bool, got {type(value).__name__}"
                    )

            elif var_type in ("string", "enum"):
                if not isinstance(value, str):
                    raise InterpreterError(
                        f"{var_id} must be string, got {type(value).__name__}"
                    )
    ...
    def _handle_decision_node(
        ...
        if on_step is not None:
            ...
            _invoke_on_step(
                on_step,
                event_payload,
                warning_message=f"on_step decision callback error at node '{node_id}'",
            )
        ...
        if not children:
            raise InterpreterError(
                f"Decision node '{node_label}' (id: {node_id}) has no children"
            )
    ...
    def _find_branch(
        self, children: List[Dict[str, Any]], condition_result: bool
    ) -> Optional[Dict[str, Any]]:
