...
def register_sio_events(
    *,
    ...
    async def _safe_handler(handler_name: str, sid: str, coro) -> None:
        """Run a handler with error catching — emit error event on failure."""
        try:
            await coro
        except Exception as exc:
            logger.error("Handler error for '%s': %s", handler_name, exc, exc_info=True)
            await sio.emit("error", {"message": f"Server error handling '{handler_name}': {exc}"}, to=sid)
    ...
    async def chat(sid: str, data: Dict[str, Any]) -> None:
        ...
        await _safe_handler("chat", sid, asyncio.to_thread(
            handle_ws_chat,
            ws_registry,
            conn_id=sid,
    ...
    async def cancel_task(sid: str, data: Dict[str, Any]) -> None:
        ...
        await _safe_handler("cancel_task", sid, asyncio.to_thread(
    ...
    async def sync_workflow(sid: str, data: Dict[str, Any]) -> None:
        ...
        await _safe_handler("sync_workflow", sid, asyncio.to_thread(
    ...
    async def resume_task(sid: str, data: Dict[str, Any]) -> None:
        ...
        await _safe_handler("resume_task", sid, asyncio.to_thread(
            handle_resume_task,
            ws_registry,
            conn_id=sid,
            user_id=user_id,
    ...
    async def execute_workflow(sid: str, data: Dict[str, Any]) -> None:
        ...
        await _safe_handler("execute_workflow", sid, asyncio.to_thread(
    ...
    async def pause_execution(sid: str, data: Dict[str, Any]) -> None:
        ...
        await _safe_handler("pause_execution", sid, asyncio.to_thread(
            handle_pause_execution, payload=data,
        ))
    ...
    async def resume_execution(sid: str, data: Dict[str, Any]) -> None:
        ...
        await _safe_handler("resume_execution", sid, asyncio.to_thread(
            handle_resume_execution, payload=data,
        ))
    ...
    async def stop_execution(sid: str, data: Dict[str, Any]) -> None:
        ...
        await _safe_handler("stop_execution", sid, asyncio.to_thread(
            handle_stop_execution, payload=data,
        ))
