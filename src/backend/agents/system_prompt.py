...
def _image_analysis_protocol() -> str:
    ...
    return (
        ...
        "- RULE: The root is the first node after Start — typically the top-most or "
        "left-most node with no incoming edges from other flowchart nodes. "
        ...
        "8. Do NOT call `update_plan` during the build — stay focused on creating "
        "nodes and connections. You will mark everything done in Step 8.\n\n"
        "**Step 8:** After ALL branches are built:\n"
        "- Verify EVERY end node has an `output` value set.\n"
        "- Call `set_workflow_output` to declare the workflow's overall output name and type.\n"
        "- Call `validate_workflow`.\n"
        "- Call `update_plan` to mark all items as done.\n\n"
        "**Step 9:** SELF-REVIEW — compare the built workflow against the original image.\n"
        "- Call `get_current_workflow` to see the full structure.\n"
        "- Call `view_image` to re-examine the original flowchart.\n"
        "- Compare node-by-node: are all nodes present? Are labels correct? "
        "Are decision conditions and true/false branches accurate? "
        "Are all end node outputs set correctly?\n"
        "- Fix obvious mistakes (wrong labels, missing connections) immediately. "
        "For ambiguous issues (unclear thresholds, interpretation choices), use `ask_question`.\n"
        "- Once satisfied, tell the user the workflow is ready.\n\n"
