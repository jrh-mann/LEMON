...
def _image_analysis_protocol() -> str:
    ...
    return (
        ...
        "**Step 2:** Call `extract_guidance` to find side notes, legends, and linked panels.\n"
        "- IDENTIFY all potential subworkflows (treatment protocols in guidance boxes, "
        "repeated decision patterns, complex multi-node clusters, calculations).\n"
        "- If you find 2+ candidates, you MUST create subworkflows for them.\n\n"
        ...
        "- RULE: The root has a UNIQUE colour or shape AND only outgoing edges "
        "(no incoming edges from other real nodes). Find it yourself — DO NOT ask the user.\n\n"
        ...
        "4. If it is a linear node (process/calculation/subprocess): continue to its single child.\n"
        ...
        "6. Use `batch_edit_workflow` when you need cross-references in the same operation.\n"
        "7. Mark each plan item done as you complete it.\n\n"
        "**Step 8:** After ALL branches are built:\n"
        "- Verify EVERY end node has an `output` value set.\n"
        "- Call `set_workflow_output` to declare the workflow's overall output name and type.\n"
        "- Call `validate_workflow`.\n\n"
        "**Step 9:** Call `highlight_node` to show progress to the user.\n\n"
        "CRITICAL RULES:\n"
