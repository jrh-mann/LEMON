...
def _data_model() -> str:
    ...
    return (
        ...
        "**CRITICAL:** If the image shows multiple paths fanning out from a single action box, "
        "you MUST convert it to a decision chain or sequential process chain. "
        "NEVER create a process/calculation node with more than one outgoing edge.\n\n"
        "### Connections\n"
...
def _subprocess_nodes() -> str:
    ...
    return (
        "## Subprocess Nodes\n\n"
        "Subworkflows are created AFTER the main graph is built and the user approves it (Step 11).\n\n"
...
def _image_analysis_protocol() -> str:
    ...
    return (
        ...
        "It is almost always a distinctive pink/salmon/rose colour — look for the uniquely coloured "
        "node at the top of the diagram. This pink node is where the flowchart begins.\n"
        ...
        "(arrow direction, pink/salmon colour, top of diagram), NOT by domain importance.\n"
        ...
        "- Do NOT stop here — you MUST continue to Steps 9 and 10.\n\n"
        "**Step 9:** MANDATORY EXECUTION TEST — you MUST test before responding.\n"
        "1. Pick 2–3 diverse test inputs that exercise different branches "
        "(e.g. one that takes the first true path, one that goes deep into false paths, "
        "one edge case).\n"
        "2. For EACH test input, BEFORE calling `execute_workflow`, mentally trace the path "
        "through the image and write down the expected output. Commit to it.\n"
        "3. Call `execute_workflow` with those input values.\n"
        "4. Compare the actual output to your expected output. If they differ, the workflow "
        "has a bug — fix it and re-test.\n\n"
        "**Step 10:** MANDATORY SELF-REVIEW — you MUST do this before responding to the user.\n"
        ...
        "**Step 11:** SUBWORKFLOW REFINEMENT (only after the user confirms the graph is correct).\n"
        ...
        "- NEVER skip Step 9 (execution test) or Step 10 (self-review). You MUST test AND visually verify before responding.\n"
...
def _anti_patterns() -> str:
    ...
    return (
        ...
        "The root is determined by STRUCTURE (pink/salmon colour, arrow direction, top of diagram), not by "
